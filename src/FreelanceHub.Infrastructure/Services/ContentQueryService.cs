    ...
    public async Task<SiteSetting> GetSiteSettingAsync(CancellationToken cancellationToken = default)
    {
        return await dbContext.SiteSettings.AsNoTracking().OrderBy(x => x.CreatedAtUtc).FirstOrDefaultAsync(cancellationToken)
            ?? new SiteSetting
            {
                ...
                AboutSummaryFr = "Template reutilisable pour consultant ou developpeur freelance, avec contenus bilingues, projets, services et experience.",
                AboutSummaryEn = "Reusable template for freelance consultants and developers, with bilingual content, projects, services, and experience.",
                SkillsFr = ".NET, ASP.NET Core, PostgreSQL, APIs, Architecture logicielle, Delivery technique",
                SkillsEn = ".NET, ASP.NET Core, PostgreSQL, APIs, Software architecture, Technical delivery",
